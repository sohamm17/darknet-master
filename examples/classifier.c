...
// return the calculation time for a single image - if needed
double folder_walk_classify_helper(network* net, char* input, int* indexes, char** names,
                                 int top) {
    double time;
    image im = load_image_color(input, 0, 0);
    image r  = im;//letterbox_image(im, net->w, net->h);
    ...
    //printf("time_now:%lf\n", what_time_is_it_now());
    float *predictions = network_predict(net, X);
    //double just_predict_time = what_time_is_it_now() - time;
    if(net->hierarchy) {
      //hierarchy_predictions(predictions, net->outputs, net->hierarchy, 1, 1);
    }
    top_k(predictions, net->outputs, top, indexes);
    //double duration_f = sec(clock() - time);//original
    double duration_f = what_time_is_it_now() - time;//original
    //printf("time_now:%lf\n", what_time_is_it_now());
    //unsigned long long duration = duration_f * 1000000000L;
    fprintf(stderr, "%s(%dx%d): Predicted in %lf seconds.\n", input, im.h, im.w, duration_f);
    //fprintf(stderr, "network_predict: %lf s, just top_k: %lf s.\n",
    //just_predict_time, duration_f - just_predict_time);
    for(i = 0; i < top; ++i) {
        int index = indexes[i];
        ...
        //else printf("%s: %f\n",names[index], predictions[index]);
        // -- SS -- changed to stderr; it was stdout
        fprintf(stderr, "%5.2f%%: %s\n", predictions[index]*100, names[index]);
    }
    if(r.data != im.data) free_image(r);
    free_image(im);
    //printf("time_now:%lf\n", what_time_is_it_now());
    return duration_f;//in seconds
}
...
void folder_walk_classify(int argc, char **argv)
{
    char* datacfg = "/home/common_shared/PAStime/darknet-master/cfg/imagenet1k.data";
    char* cfgfile = argv[2];
    char* weightfile = argv[3];
    char*  dirname = argv[4]; // this is directory
    int top = 5;
    int num_files = atoi(argv[5]);
    int total_time_th = atoi(argv[6]);
    int time_interval = 1; // 5 seconds - need to be taken as argument

    static lt_t exec_cost, exec_cost_hi, period, r_lo, r_star;
    int prio;

    exec_cost = atoi(argv[7]);
    exec_cost_hi = atoi(argv[8]);
    period = atoi(argv[9]);
    prio = atoi(argv[10]);
    r_lo = atoi(argv[11]);
    r_star = atoi(argv[12]);
    ...
    char *name_list = option_find_str(options, "names", 0);
    if(!name_list) name_list = option_find_str(options, "labels",
      "/home/common_shared/PAStime/darknet-master/data/labels.list");
    ...
    //char buff[256];
    //char *input = buff;
    char** input = (char *) malloc(sizeof(char*) * num_files);
    ...
    if(dr == NULL) {
      perror("folder_walk_classify:");
      exit(-1);
    }

    //int half_files = num_files/2;
    int half_files = num_files; //just running a single loop
    
    int cp1 = num_files / 2 /*6, cp2 = (2 * num_files) / 6, cp3 = (3 * num_files)
    / 6, cp4 = (4 * num_files) / 6, cp5 = (5 * num_files) / 6*/;

    double timer_start = what_time_is_it_now();
    int tot_images = 0;

    while(half_files-- > 0 && (de = readdir(dr)) != NULL) {
      ...
      if(filename){
          strncpy(input[tot_images], filename, 256);
      }
      tot_images++;
      
      if (filename && !num_files) break;
    }
    // release_ts from here
    int i = 0;
    fp_time_log = fopen("filetime.log", "a+");
    lt_t finish_times[tot_images];
    lt_t release_times[tot_images];
    lt_t new_r_los[tot_images];
    double cputimes[tot_images];
    double clocktimes[tot_images];
    int iterations[tot_images];
    int sched[tot_images];
    ...
    /*for(i = 0; i < final_no_img; i++) {
      lt_t new_r_lo = new_r_los[i];//r_lo + extra_cost_arr[i];
      
      lt_t job_time = ns2ms(finish_times[i] - release_times[i]);
      int hi_mode = (job_time > new_r_lo ? 1 : 0);
      
      printf("%d, %d, %.4lf, %.4lf, %d\n", params.priority, i + 1, (double)
      job_time / 1000, (double) new_r_lo / 1000, hi_mode);
      tot_hi_mode += hi_mode;
      if(sched[i] >= 0) {
        printf("Sched, %d, Iter, %d\n", sched[i], iterations[i]);
      }
    }*/
    //printf("HI-Switch--\n");
    /*for(i = 1; i < final_no_img - 4; i++) {
      if(new_r_lo > ns2ms(params.period))
        new_r_lo = ns2ms(params.period);
      printf("%d, %.4lf\n", i, (double) new_r_lo / 1000);
    }*/
...
void predict_classifier(char *datacfg, char *cfgfile, char *weightfile, char *filename, int top)
{
    ...
    while(1){
        ...
        float *X = r.data;
        // -- SS -- instrumented my own getTime for nanosecond output in stdout
        //time=clock(); //original
        time=what_time_is_it_now();
        float *predictions = network_predict(net, X);
        ...
        double duration_f = what_time_is_it_now() - time;//original
        unsigned long long duration = duration_f * 1000000000L;
        fprintf(stderr, "%s: Predicted in %lf seconds.\n", input, duration_f);
